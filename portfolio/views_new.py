...
class SecurityViewSet(viewsets.ModelViewSet):
    ...
    def import_security(self, request):
        ...
        serializer = SecuritySerializer(result['stock'])
        return Response({
...
def portfolio_holdings_consolidated(request, portfolio_id):
    """Get consolidated view of holdings grouped by security"""
    try:
    ...
    for security_id, data in holdings.items():
        ...
        for transaction in data['transactions']:
            if transaction.transaction_type in transactions_by_type:
                transactions_by_type[transaction.transaction_type].append(
        ...
        holding_data = {
            ...
            'total_dividends': float(data['total_dividends']),
            'gain_loss_pct': float(
                (data['unrealized_gains'] / (data['quantity'] * data['avg_cost']) * 100) if data['quantity'] > 0 and
                                                                                            data[
                                                                                                'avg_cost'] > 0 else 0),
            'transactions': {
                'all': TransactionSerializer(data['transactions'], many=True).data,
